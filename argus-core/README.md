...
## Live image-to-mission pipeline (simulation or authorised camera only)

`run_vision_pipeline.py` is the end-to-end entry point when you have a
before/after PNG or JPEG pair. It sends the image pair to Gemini
Vision, converts a returned normalised bounding box through **simulator-supplied
georeferencing**, builds and validates a mission, and appends events for the
dashboard:

```text
before/after frames -> Gemini Vision -> Detection -> MissionSpec -> FlightPlan
-> ValidationResult -> dispatch_requested event (accepted plans only)
```

### Standalone: no environment-team dependency

Create `input/before.png` and `input/after.png`, then run:
...
The standalone mode derives a healthy simulated drone and maps the entire
screenshot to the included fictional policy geofence. It is enough to show the
end-to-end pipeline independently. See `input/README.md`.

### Later: accurate simulator mapping

When the environment team is ready, provide `frame_meta.json` in the shape of
`fixtures/frame_meta.example.json`, plus a drone-state JSON object. The
simulator's image-to-world mapping replaces the standalone approximation.
...
python3 run_vision_pipeline.py \
  ...
  --frame-meta fixtures/frame_meta.example.json \
  --drone fixtures/drone.example.json --policy fixtures/policy.json
```

Events go to `runs/dashboard_events.jsonl` by default. The dashboard or
simulator controller should consume `dispatch_requested` and execute a plan
only after it sees `validation.verdict == "accept"`.

This project is simulation/authorised-site software. It does not ingest or
target data from sensitive real facilities, and it does not directly command a
physical drone.

Gemini calls use the current Interactions API rather than direct automatic
function-calling through `models.generate_content`. Update the SDK before the
first live run:
