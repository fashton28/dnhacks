# Eye in the Sky — Firmware Flashing & Software Deployment
...
git clone https://github.com/<your-repo>/eyeinthesky.git /opt/eyeinthesky
cd /opt/eyeinthesky
...
```bash
sudo cp scripts/eyeinthesky-companion.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable eyeinthesky-companion.service
sudo systemctl start eyeinthesky-companion.service
sudo systemctl status eyeinthesky-companion.service
```

The service file (`scripts/eyeinthesky-companion.service`) should contain:

```ini
[Unit]
Description=Eye in the Sky Companion
...
Restart=on-failure
RestartSec=5
WorkingDirectory=/opt/eyeinthesky
...
docker exec -it eyeinthesky-companion bash
...
docker logs eyeinthesky-companion 2>&1 | grep -E "MAVLink|UART|serial"
...
# ground/dist/Eye in the Sky Setup-<version>.exe
```
