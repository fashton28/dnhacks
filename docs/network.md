...
# Create a WiFi hotspot named "EyeInTheSky"
sudo nmcli con add type wifi ifname wlan0 con-name "EyeInTheSky-AP" \
    ssid "EyeInTheSky" \
    mode ap \
    ipv4.method shared \
...
# Activate it
sudo nmcli con up "EyeInTheSky-AP"

# Make it auto-start on boot
sudo nmcli con modify "EyeInTheSky-AP" connection.autoconnect yes
```
...
- Ground PC (connected to "EyeInTheSky" network): auto-assigned **192.168.4.2–254** by DHCP
...
2. Select network **"EyeInTheSky"** and enter the passphrase.
...
3. The Eye in the Sky Electron app should be auto-allowed when first run; if it prompts
...
In the Eye in the Sky ground control center:
