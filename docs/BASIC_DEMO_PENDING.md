...
| Teammate topology and world | No committed Webots world or final `site/site.json` was found in the fetched `main` tree. | Use the labelled Meridian Station stub and staged assets for the basic demo. Replace them only when the teammate site contract is available. Unpublished teammate work cannot be assessed from Git. |
...
| Two runtimes now implement the basic demo: the ARGUS stack (`hub/`, `console/`, `sim/`, Three.js, ARGUS site) and the retrofit (`platform/`, Electron, Meridian Station site). | Pick one for the basic demo and say which here. They do not share contracts, site models, or simulators, and there is no adapter between them. |
| Both runtimes now describe the same place: `platform/site/site.stub.json` is generated from the ARGUS Meridian Station sources. | Run one named Meridian Station scenario. The two stacks still keep separate site FILES, so do not mix coordinates or exclusion rules across them without regenerating the stub. |
