 ...
 */
export function Compass({ heading = 0, size = 200, target = null, label = true }) {
  const r = size / 2;
  const ticks = [];
      ...
      <svg width={size} height={size} viewBox={`${-r} ${-r} ${size} ${size}`}>
        ...
        <g transform={`rotate(${-heading})`} style={{ transition: 'transform var(--needle-ease) 120ms' }}>
          ...
          {target != null && (() => {
            ...
            return <polygon points="0,-7 -5,3 5,3" fill="var(--accent)" transform={`translate(${Math.cos(a) * (r - 6)} ${Math.sin(a) * (r - 6)}) rotate(${target})`} />;
          })()}
        </g>
