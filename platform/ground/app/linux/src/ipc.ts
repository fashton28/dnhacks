 ...
 *   power:inhibit       invoke  ()               → void  (Linux: hold a powerSaveBlocker)
 *   power:release       invoke  ()               → void  (Linux: drop it)
 *
 ...
 * process state. The Linux delta vs ../../windows/src/ipc.ts is the
 * `PowerInhibitor` and its two channels (LINUX_PRD §7).
 */
...
/* ── power:inhibit / power:release (LINUX_PRD §7) ───────────────────────── */

/** The slice of Electron's powerSaveBlocker the inhibitor needs (injectable). */
...
export interface PowerInhibitor {
  /** Idempotent: a second inhibit() while one is held changes nothing. */
  inhibit(): void;
  /** Idempotent: releasing with nothing held is a no-op. */
  release(): void;
  readonly held: boolean;
}

/**
 * Keeps at most ONE display-sleep blocker alive. The renderer drives it from
 * telemetry (armed / tracking / manual / planner active → inhibit, otherwise
 * release); on Linux Electron maps the blocker onto the freedesktop /
 * systemd-inhibit machinery, elsewhere it is a harmless no-op.
 */
export function createPowerInhibitor(blocker: PowerSaveBlockerLike): PowerInhibitor {
  let token: number | null = null;
  const live = (): boolean => token !== null && blocker.isStarted(token);
  return {
    get held() {
      return live();
    },
    inhibit() {
      if (live()) return;
      token = blocker.start('prevent-display-sleep');
    },
    release() {
      if (live()) blocker.stop(token as number);
      token = null;
    },
  };
}
...
export function registerIpcHandlers(): void {
  ...
  const power = createPowerInhibitor(powerSaveBlocker);
  ...
  const requests: Record<string, InvokeHandler> = {
    ...
    'power:inhibit': () => { power.inhibit(); },
    'power:release': () => { power.release(); },
  };
