# Drone Safety Platform — Ground Control Center (Linux shell)
...
| Area | Mechanism |
|---|---|
| **Settings / recordings** | XDG dirs automatically — `app.getPath('userData')` → `~/.config/Drone Safety Platform/`; recordings under there or `EIS_RECORDINGS_DIR` (e.g. `$XDG_DATA_HOME/dnhacks-platform`). No code change vs. Windows. |
| **Game controllers** | udev rule (`99-dnhacks-platform-input.rules`) installed by the `.deb`/`.rpm` postinstall; AppImage/Flatpak users install it manually or join group `input`. |
