...
/* ── Linux power inhibit (LINUX_PRD §7) ─────────────────────────────────── */

describe('power inhibitor', () => {
  test('holds at most one blocker and releases idempotently', () => {
    const blocker = fakeElectron.powerSaveBlocker;
    powerLog.length = 0;
    const power = ipcMod.createPowerInhibitor(blocker);
    assert.equal(power.held, false);
    power.release();                       // nothing held → no-op
    power.inhibit();
    power.inhibit();                       // already held → no second blocker
    assert.equal(power.held, true);
    assert.deepEqual(powerLog.filter(([op]) => op === 'start').map(([, kind]) => kind), ['prevent-display-sleep']);
    power.release();
    power.release();
    assert.equal(power.held, false);
    assert.equal(blocker._live.size, 0);
    assert.equal(powerLog.filter(([op]) => op === 'stop').length, 1);
    power.inhibit();                       // can be re-armed after a release
    assert.equal(power.held, true);
    power.release();
  });

  test('a blocker stopped behind our back is re-acquired on the next inhibit', () => {
    const blocker = fakeElectron.powerSaveBlocker;
    const power = ipcMod.createPowerInhibitor(blocker);
    power.inhibit();
    const [id] = [...blocker._live];
    blocker.stop(id);                      // e.g. the session manager dropped it
    assert.equal(power.held, false);
    power.inhibit();
    assert.equal(power.held, true);
    power.release();
