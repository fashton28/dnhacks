...
import { Video, Globe, Crosshair, SlidersHorizontal, Radar, ScrollText, ShieldCheck } from 'lucide-react';
import { useArgus } from '../store';

export type View = 'flight' | 'world' | 'mission';
export type DrawerId = 'ops' | 'fleet' | 'log' | 'safety';
    ...
    <nav className="a-glass a-rail" aria-label="Views">
      ...
      {DRAWERS.map((d) => (
        <button key={d.id} className="a-railbtn" data-on={drawer === d.id} onClick={() => onDrawer(drawer === d.id ? null : d.id)} title={d.title}
          data-badge={d.id === 'safety' && rejected ? '!' : undefined}>{d.icon}<span>{d.label}</span></button>
      ))}
    </nav>
  );
}
