...
function Mini({ label, value, span2 = false }: MiniProps) {
  return (
    <div style={{
      display: 'flex',
...
function Spark({ label, data, unit, color }: SparkProps) {
  const w = 252;
  const h = 34;
  ...
  const rng = max - min || 1;
  const pts = vals
    .map((v, i) => `${(i / Math.max(1, vals.length - 1)) * w},${h - ((v - min) / rng) * (h - 4) - 2}`)
    .join(' ');
...
export function TelemetryPanel({ tel, tracking, history }: TelemetryPanelProps) {
  ...
  const batRemaining = bat?.remaining ?? 100;
  const batStatus = batRemaining <= 15 ? 'danger' : batRemaining <= 30 ? 'caution' : 'nominal';
