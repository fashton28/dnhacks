# PRD — Eye in the Sky GCS: Windows → Linux port
...
- Ship a `.desktop` file (icon from `assets/logo-mark.svg` rasterized to standard
  sizes), categories `Utility;Science;`, stable app-id `com.eyeinthesky.gcs`.
...
- Confirm Xbox (xpad), PS4/5 (`hid-playstation`/hid-sony), and generic HID pads
  enumerate. Ship a **udev rule** (`/etc/udev/rules.d/99-eyeinthesky-input.rules`,
...
- `SettingsStore` + flight recordings use **XDG base dirs** (`$XDG_CONFIG_HOME`,
  ...
  Recordings default under `$XDG_DATA_HOME/eyeinthesky/`.
